...
def train_models_with_comparison(**context):
    ...
    try:
        ...
        logging.info(f"🏷️ Class distribution: {y.value_counts().to_dict()}") 
        ...
        if not predictions_df.empty: 
            with pg_conn.cursor() as cursor: # <--- Perubahan di sini: Menggunakan cursor dari pg_conn
                ...
                data_to_insert = [
                    (row.earthquake_id, row.classified_risk_zone, row.classification_confidence,
                     row.model_version, row.created_at)
        ...
        if not model_comparison.empty: # Memastikan ada data untuk disimpan
            with pg_conn.cursor() as cursor: 
                ...
                """
                metadata_to_insert = [
                    (metrics['model_type'], metrics['model_type'], metrics['accuracy'],
                     metrics['precision'], metrics['recall'], metrics['f1_score'],
                     metrics['training_samples'], datetime.now()) # Sesuaikan kolom
